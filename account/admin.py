...
from account.models import CostCenter, Department, EmployeeRole, CompanyCode, OfficeBranch
...
class EmployeeModelAdmin(BaseUserAdmin):
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('email', 'name', 'empID', 'password1', 'password2', 'department','companyCode', 'branch',
                       'is_verified', 'role', 'costcenter','doj', 'dol',),
        }),
    )


    search_fields = ('email','name','empID','department','companyCode')
...
class CostCenterAdmin(admin.ModelAdmin):
    list_display = ['id','name', 'number','company_code']
